...
Ứng dụng web giúp ôn tập câu hỏi trắc nghiệm từ file Markdown. Cách dùng khuyến nghị: **VS Code + extension Live Preview** để mở app, **chụp màn hình từng cụm câu → Gemini trích xuất → làm lại trên app** để vừa nhập đề vừa tự kiểm tra.
...
```
Đề gốc (web/PDF)          VS Code                    Gemini
      │                        │                              │
      │  Chụp màn 5–10 câu     │                              │
      ├───────────────────────►│  prompt.md + đề.md + ảnh ──► │  Trích xuất .md
      │                        │                              │
      │                        │  Dán vào file .md, lưu       │
      │                        │◄─────────────────────────────┤
      │                        │                              │
...
| **2. Gửi Gemini** | Mở **[Gemini](https://gemini.google.com/)** (ưu tiên — đọc ảnh chụp màn tốt). Dán nội dung `prompt.md` (hoặc [Prompt ngắn](#trích-xuất-câu-hỏi-từ-pdfảnh)), **đính kèm file đề `.md` hiện có** (ví dụ `2020-2/2020..2.md`) và **ảnh chụp màn** |
...
| **6. Import lại** | **Sau khi sửa file `.md`, bắt buộc import lại** — reload **Live Preview** (đề trong `manifest.json`) hoặc kéo-thả/chọn file lại trên sidebar (đề import thủ công) để app nạp nội dung mới và kiểm tra format chuẩn |
| **7. Tự test trên app** | Chọn đề → **làm lại từng câu vừa nhập** → bấm Kiểm tra |
| **8. Lặp** | Chụp cụm 5–10 câu tiếp theo, append vào cùng file `.md`, import lại và làm tiếp |
...
- Giữ **Live Preview** và tab **Gemini** mở song song: chụp → gửi `prompt.md` + file `.md` + ảnh → dán output → **import lại** (reload preview hoặc kéo-thả file)

### Sau khi sửa file `.md` — import lại

Mỗi lần chỉnh sửa nội dung đề (thêm câu, sửa `ans:`, đổi ảnh…), app **không tự cập nhật** nếu bạn chỉ lưu file trên đĩa. Cần **import lại** để parser đọc bản mới và đảm bảo format chuẩn:

| Cách load đề | Cách import lại |
|--------------|-----------------|
| Đề trong `manifest.json` | **Reload Live Preview** (đóng/mở preview hoặc refresh trang preview) |
| Đề kéo-thả / chọn file thủ công | **Kéo-thả** hoặc bấm **Import** và chọn lại file `.md` trên sidebar |

Nếu không import lại, bạn có thể vẫn thấy nội dung cũ (cache trong `localStorage`) hoặc bỏ sót lỗi format cho đến khi làm bài trên app.
...
| **`prompt.md`** | Prompt gửi AI khi chụp màn hình đề (kèm file `.md` đề hiện có) |
| **Gemini** | Trích xuất 5–10 câu/lần từ ảnh chụp màn — **ưu tiên dùng Gemini** |
...
- Tự động nạp đề từ `manifest.json` khi mở qua Live Preview
...
2. **File → Open Folder** → chọn thư mục `MachineLearning`
3. Mở `index.html` → **Show Preview**

Live Preview tự chạy server cục bộ → `manifest.json` và ảnh `assets/` load đầy đủ.
...
### Bước 3 — Import lại và kiểm tra

1. Mở **Live Preview** (xem [Chạy project](#chạy-project))
2. **Import lại** sau khi sửa `.md`: reload preview (đề trong manifest) hoặc kéo-thả/chọn file lại (import thủ công) — xem [Sau khi sửa file `.md`](#sau-khi-sửa-file-md--import-lại)
3. Đề mới xuất hiện ở sidebar (nếu vừa thêm vào `manifest.json`)
4. **Làm lại từng câu vừa thêm** — đây là bước tự test bắt buộc trong workflow
5. Kiểm tra ảnh hiển thị và đáp án đúng/sai
...
File **`prompt.md`** chứa hướng dẫn đầy đủ để gửi **Gemini**. Cách dùng thực tế nhất: **chụp màn hình 5–10 câu**, kèm **file đề `.md` đang làm** (ví dụ `2020-2/2020..2.md`), không upload cả file PDF một lần.

### Quy trình (khớp workflow vừa làm vừa học)

1. **Chụp màn hình** 5–10 câu từ đề gốc (Windows: `Win+Shift+S` · macOS: `Cmd+Shift+4`)
2. Mở **[Gemini](https://gemini.google.com/)** — ưu tiên dùng Gemini để trích xuất từ ảnh chụp màn
3. Dán nội dung `prompt.md` — hoặc prompt ngắn bên dưới
4. **Đính kèm file đề `.md` hiện có** (ví dụ `2020-2/2020..2.md`) — để AI biết format và số câu đã trích xuất, append đúng chỗ
5. **Đính kèm ảnh chụp màn** (paste trực tiếp hoặc upload)
6. Copy output `.md` → dán vào file trong VS Code
7. Lưu → **import lại** (reload **Live Preview** hoặc kéo-thả file) → **làm lại các câu vừa nhập** để tự test
8. Lặp với cụm câu tiếp theo

### Prompt ngắn (copy-paste nhanh)

Gửi kèm **file đề `.md` hiện có** (ví dụ `2020-2/2020..2.md`) và **ảnh chụp màn**, rồi dán prompt:
...
- **Import lại** file sau khi lưu (reload Live Preview hoặc kéo-thả lại) — bắt buộc để app nạp bản mới
...
> **Quan trọng — phạm vi đóng góp:** Chỉ được phép **tạo thêm đề** (file `.md`, thư mục đề, `assets/`, cập nhật `manifest.json`). **Không được tự ý sửa `index.html`.** Muốn thay đổi giao diện, tính năng app, hoặc logic parser — **liên hệ chủ repo** trước (mở issue hoặc nhắn trực tiếp).
...
- [ ] Chỉ thêm/sửa file đề (`.md`, `assets/`, `manifest.json`) — **không sửa `index.html`**
...
- [ ] Đã **import lại** file `.md` sau khi sửa (reload Live Preview hoặc kéo-thả lại)
- [ ] Đã test qua **Live Preview** — làm lại từng câu, ảnh hiện, đáp án đúng

### 2. Ứng dụng web (`index.html`) — không tự sửa

Toàn bộ frontend nằm trong một file `index.html`. **Người đóng góp không được tự ý chỉnh sửa file này** (kể cả sửa nhỏ UI, thêm tính năng, hay sửa parser).

Nếu bạn thấy bug app, muốn tính năng mới, hoặc cần thay đổi cách hiển thị/làm bài — **báo chủ repo** để được xử lý hoặc thống nhất trước khi merge.

*(Chủ repo giữ quyền sửa `index.html`; khi sửa cần giữ tương thích format `.md`, không thêm build step, test trên Chrome/Firefox/Safari nếu đổi UI lớn.)*
...
# ... chỉnh sửa file .md, assets/, manifest.json — KHÔNG sửa index.html ...
...
- Cần **Live Preview** để auto-load `manifest.json` và ảnh bundled
...
- Gặp lỗi parse: kiểm tra format `.md` theo [Format file câu hỏi](#format-file-câu-hỏi-md), rồi **import lại** file
...
- Muốn sửa `index.html`: **liên hệ chủ repo** — không tự ý commit thay đổi app
